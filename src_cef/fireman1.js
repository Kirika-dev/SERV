...
const streamedFireObjectList=new Set,MAX_RANGE=100;
class FireObject {
    constructor(a,b,c) {
    ...
    0===streamedFireObjectList.size||(streamerInterval=setInterval(async()=> {
        ...
        for(const a of streamedFireObjectList) {
            ...
            if(-1!==a.fireFxId) {
                ...
                if(!e&&(e=!0,!mp.game.streaming.hasNamedPtfxAssetLoaded("core")))for(mp.game.streaming.requestNamedPtfxAsset("core");
...
let firemanWorkFxId=-1,fireObjectStartRemove=null,fireObjectRemoveTime=0,fireObjectRemoveLastTime=0,firemanSyncLast=0,fireextinguisherLastShootPos=new mp.Vector3(0,0,0),fireextinguisherLastShootTime=0;
const eventRenderFiremanWork=async()=> {
    const a=mp.players.local;
    if(a.vehicle)return void mp.game.controls.disableControlAction(0,68,!0);
    const b=getTime();
    let c=null,d=0,e=null,f=null,g=10,h=0;
    ...
    if(i&&f&&0<h&&mp.game.player.isFreeAiming()) {
        if(mp.game.controls.disableControlAction(0,24,!0),mp.game.controls.disableControlAction(1,24,!0),mp.game.controls.disableControlAction(0,257,!0),-1===firemanWorkFxId) {
            if(mp.attachmentMngr.addClient(a,mp.game.joaat("prop_hose_nozzle")),!mp.game.streaming.hasNamedPtfxAssetLoaded("core"))for(mp.game.streaming.requestNamedPtfxAsset("core");
            !mp.game.streaming.hasNamedPtfxAssetLoaded("core");
            )await mp.game.waitAsync(0);
        ...
        const g=gameplayCamera.getCoord(),h=gameplayCamera.getDirection(),i=new mp.Vector3(20*h.x+g.x,20*h.y+g.y,20*h.z+g.z),j=mp.raycasting.testPointToPoint(g,i,mp.players.local,31),k=j&&j.position?j.position:new mp.Vector3(i.x,i.y,mp.game.gameplay.getGroundZFor3dCoord(i.x,i.y,i.z+1,0,!1));
        mp.game.graphics.drawMarker(28,k.x,k.y,k.z,0,0,0,0,0,0,.5,.5,.5,255,0,0,100,!1,!1,2,!1,"","",!1),c=k,d=2,e=f,firemanSyncLast+450<b&&(firemanSyncLast=b,mp.events.callRemote("server_fireman_sync"))
    }
    else {
        if(i)if(f)0<h?mp.game.graphics.drawText("\u0412\u044B \u043C\u043E\u0436\u0435\u0442\u0435 \u0438\u0441\u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u044C \u0432\u043E\u0434\u0443 \u0434\u043B\u044F \u0442\u0443\u0448\u0435\u043D\u0438\u044F \u043F\u043E\u0436\u0430\u0440\u0430 ( \u041F\u041A\u041C )",[.5,.9], {
            font:4,centre:!0,color:[255,255,255,255],scale:[.4,.4],outline:!1
        })
        ...
        -1!==firemanWorkFxId&&(mp.attachmentMngr.removeFor(a,mp.game.joaat("prop_hose_nozzle")),mp.game.graphics.removeParticleFx(firemanWorkFxId,!1),firemanWorkFxId=-1),fireextinguisherLastShootTime+100>b&&(c=fireextinguisherLastShootPos,d=1)
    }
    if(c) {
        let a=null,f=1.5;
        for(const b of streamedFireObjectList) {
            const d=mp.dist(c.x,c.y,c.z,b.position.x,b.position.y,b.position.z);
            d<f&&(a=b,f=d)
        }
        if(null==a)return;
        if(a!==fireObjectStartRemove)fireObjectStartRemove=a,fireObjectRemoveTime=0,fireObjectRemoveLastTime=b;
        else if(fireObjectRemoveLastTime+2500>b) {
            fireObjectRemoveLastTime=b,fireObjectRemoveTime+=d,fireObjectRemoveTime>=a.getTimeToRemove()&&(fireObjectRemoveTime=0,mp.events.callRemote("server_fire_remove",a.id,e),global.discordUpdate("\u0422\u0443\u0448\u0438\u0442 \u043F\u043E\u0436\u0430\u0440",120));
            const f=mp.game.graphics.world3dToScreen2d(c.x,c.y,c.z);
            f&&(mp.game.graphics.drawRect(f.x,f.y,.03+.002,.006+.002,0,0,0,200),mp.game.graphics.drawRect(f.x,f.y,.03,.006,41,66,78,255),mp.game.graphics.drawRect(f.x-.03/2*(1-fireObjectRemoveTime/a.getTimeToRemove()),f.y,.03*(fireObjectRemoveTime/a.getTimeToRemove()),.006,48,108,135,200))
        }
...
const eventPlayerWeaponShotFiremanWork=a=> {
    if(101631238===mp.game.invoke("0x0A6DB4965674D243",mp.players.local.handle)&&a) {
        const b=getTime();
        firemanFireextinguisherSyncLast+450<b&&(firemanFireextinguisherSyncLast=b,mp.events.callRemote("server_fireman_fireext_sync")),fireextinguisherLastShootPos=a,fireextinguisherLastShootTime=b
    }
};
...
mp.events.add("fireman.sync",async a=> {
    ...
    if(b&&0!==b.handle&&(b.firemanSyncLast=getTime(),!b.firemanSyncTimer)) {
        if(!mp.game.streaming.hasNamedPtfxAssetLoaded("core"))for(mp.game.streaming.requestNamedPtfxAsset("core");
        !mp.game.streaming.hasNamedPtfxAssetLoaded("core");
        )await mp.game.waitAsync(0);
...
mp.events.add("fireman.ext.sync",async a=> {
    const b=mp.players.atRemoteId(a);
    if(b&&0!==b.handle&&(b.firemanSyncLast=getTime(),!b.firemanSyncTimer)) {
        if(!mp.game.streaming.hasNamedPtfxAssetLoaded("core"))for(mp.game.streaming.requestNamedPtfxAsset("core");
        !mp.game.streaming.hasNamedPtfxAssetLoaded("core");
        ...
        if(!mp.players.exists(b)||0===b.handle)return void mp.game.streaming.removeNamedPtfxAsset("core");
        mp.game.graphics.setPtfxAssetNextCall("core");
        const a=mp.game.graphics.startParticleFxLoopedOnPedBone("weap_extinguisher",b.handle,0,0,0,88,290,120,6286,.8,!1,!1,!1),c=mp.game.graphics.startParticleFxLoopedOnPedBone("weap_extinguisher",b.handle,0,0,0,88,290,120,6286,.8,!1,!1,!1),d=mp.game.graphics.startParticleFxLoopedOnPedBone("weap_extinguisher",b.handle,0,0,0,88,290,120,6286,.8,!1,!1,!1);
        b.firemanSyncTimer=setInterval(()=> {
            if(!mp.players.exists(b)||0===b.handle||1e3<getTime()-b.firemanSyncLast)return mp.game.graphics.removeParticleFx(a,!1),mp.game.graphics.removeParticleFx(c,!1),mp.game.graphics.removeParticleFx(d,!1),clearInterval(b.firemanSyncTimer),void delete b.firemanSyncTimer
        },
        100),mp.game.streaming.removeNamedPtfxAsset("core")
    }
});
